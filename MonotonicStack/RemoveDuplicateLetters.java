...
public class RemoveDuplicateLetters {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<c.length;i++) {
			if(freq[c[i]-'a']>0) continue;
			freq[c[i]-'a']++;
			while(!stack.isEmpty()&&c[i]-'a'<=stack.peek()-'a'&&i<lastInd[stack.peek()-'a']) {
				freq[stack.pop()-'a']--;
			}
			stack.push(c[i]);
		}
		...
		while(stack.isEmpty()==false) {
			reStringBuilder.append(stack.pop());
		}
		reStringBuilder=reStringBuilder.reverse();
		String ans=reStringBuilder.toString();
		p.println(ans);
