package NovemberLeetcodingChallange;
...
public class Two2 {
	...
	static int dfs(int mat[][],int a,int b,int x,int y,boolean vis[][]) {
		
		if(a<0||b<0||a>=mat.length||b>=mat[0].length||mat[a][b]==-1||vis[a][b]==true) return 0;
		if(a==x&&b==y) {
			if(check(mat,vis)) return 1;
		}
		vis[a][b]=true;
		int ans=0;
		for(int i=0;i<4;i++) {
			ans+=dfs(mat, a+offx[i],b+offy[i], x, y, vis);
		}
		vis[a][b]=false;
	...
	static boolean check(int mat[][],boolean vis[][]) {
