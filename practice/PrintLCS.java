...
public class PrintLCS {
	...
	public static void main(String[] args) {
		...
		p.println(str.reverse().toString());
		p.close();

	}

	static int solve(int dp[][], char s[], char t[], int i, int j) {
		if (dp[i][j] != -1)
			return dp[i][j];
		if (i == 0 || j == 0)
			return 0;
		...
		if (s[i - 1] == t[j - 1]) {
			return dp[i][j] = 1 + solve(dp, s, t, i - 1, j - 1);
		} else {
			return dp[i][j] = Math.max(solve(dp, s, t, i - 1, j), solve(dp, s, t, i, j - 1));
		}
	}
