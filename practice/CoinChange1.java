...
public class CoinChange1 {
	...
	public static void main(String[] args) {
		...
		int res=solve(dp,coins,0,amt);
		p.println(res);
		p.close();

	}
	
	static int solve(int dp[][],int coins[],int i,int amt) {
		...
		if(coins[i]<=amt) {
			return dp[i][amt]=solve(dp,coins,i,amt-coins[i])+solve(dp,coins,i+1,amt);
		}else {
			return dp[i][amt]=solve(dp,coins,i+1,amt);
		}
