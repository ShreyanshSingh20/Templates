...
public class LongestSubsequenceRepeatedkTimes {
    ...
    public static void main(String[] args) {
        ...
        for(int len=total;len>=0;len--) {
        	generate(candidates, freq, 0, len, new StringBuilder());
        }
        
        boolean found=false;
        StringBuilder res = new StringBuilder("");
        for(String s:candidates) {
        	if(check(s.toCharArray(), c, k)) {
        		found=true;
        		res.append(s);
        		break;
        	}
        ...
        p.println(res.toString());
        
        p.close();

    }
    
    static boolean check(char c[],char s[],int k) {
    	if(c.length==0) return true;
    	int pt=0;
    	int cnt=0;
    	for(int i=0;i<s.length;i++) {
    		if(s[i]==c[pt]) pt++;
    		if(pt>=c.length) { cnt++; pt=0;}
    	}
    	
    	return cnt>=k;
    }
    
    static void generate(List<String> candiList,int freq[],int curr,int len,StringBuilder s) {
    	
    	if(curr==len) {
    		candiList.add(s.toString());
    		return;
    	}
    	...
    	for(int i=25;i>=0;i--) {
    		if(freq[i]>0) {
    			s.append((char)(i+'a'));
    			freq[i]--;
    			generate(candiList,freq, curr+1, len, s);
