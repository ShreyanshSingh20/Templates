...
public class BellmanFord {
	...
	public static void main(String[] args) {
		...
		int n=f.nextInt();
		int e=f.nextInt();
		long mat[][]=new long[e][3];
		for(int i=0;i<e;i++) {
			mat[i][0]=f.nextLong()-1;
			mat[i][1]=f.nextLong()-1;
			mat[i][2]=f.nextLong();
		}
		BellManFord(mat, n, e);
		p.close();
	...
	static void BellManFord(long mat[][], int n, int e) {
		long sp[] = new long[n];
		final long inf=Long.MAX_VALUE;
		Arrays.fill(sp, inf);
		final long ninf = -1*inf;
		sp[0] = (long)0;
		...
		for (int i = 0; i < n - 1; i++) {
			for (int j = 0; j < e; j++) {
				...
				if (sp[(int) s] == inf)
					continue;
				if (sp[(int) s] + w < sp[(int) d]) {
					sp[(int)d]=sp[(int)s]+w;
				}
				sp[(int) d] = Math.max(sp[(int) d], ninf);
			}
		}
		int x=-1;//if neg cycle present
		for (int j = 0; j < e; j++) {
			long s = mat[j][0];
			long d = mat[j][1];
			long w = mat[j][2];
		...
		//to print the neg cycle in the parent array at the nth iteration store which
		//node is updating and backtrack using the parent array

		if (x != -1) {
			System.out.println("YES! neg cycle found");
		}
		else System.out.println(sp[n - 1]);
	}
