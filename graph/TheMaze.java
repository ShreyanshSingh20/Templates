...
public class TheMaze {
	...
	public static void main(String[] args) {
		...
		p.println(dfs(sx, sy, di, dj, mat));
		p.println(bfs(sx, sy, di, dj, mat,vis));

		p.close();

	}

	static boolean dfs(int i,int j,int di,int dj,int mat[][]) {
		
		if(i<0||j<0||i>=mat.length||j>=mat[0].length||mat[i][j]==1) return false;
		if(i==di&&j==dj) return true;
		mat[i][j]=1;
		if(dfs(i+1, j, di, dj, mat)) return true;
		if(dfs(i, j+1, di, dj, mat)) return true;
		if(dfs(i-1, j, di, dj, mat)) return true;
		if(dfs(i, j-1, di, dj, mat)) return true;
		mat[i][j]=0;
	...
	static boolean bfs(int i,int j,int di,int dj,int mat[][],boolean vis[][]) {
		...
		while(!queue.isEmpty()) {
			...
			for(int k=0;k<size;k++) {
				...
				for(int z=0;z<4;z++) {
					int cx=currx+offx[z];
					int cy=curry+offy[z];
					if(cx<0||cy<0||cx>=mat.length||cy>=mat[0].length||mat[cx][cy]==1) continue;
					if(cx==di&&cy==dj) return true;
					vis[cx][cy]=true;
