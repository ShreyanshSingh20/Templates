...
import graph.CycleDetectionUndirected.Pair;
...
public class CycleDetectionDirectedGraph {
	...
	static boolean dfsChecker(ArrayList<ArrayList<Integer>> g, int v) {
		boolean vis[] = new boolean[v];
		boolean backEdge[] = new boolean[v];
		for (int i = 0; i < v; i++) {
			if (vis[i] == false) {
				if (dfsSolver(g, i, vis, backEdge))
					return true;
			}
		}
	...
	static boolean dfsSolver(ArrayList<ArrayList<Integer>> g, int s, boolean vis[], boolean backEdge[]) {
		vis[s] = true;
		backEdge[s] = true;
		for (int next : g.get(s)) {
			if (vis[next] == false) {
				if (dfsSolver(g, next, vis, backEdge))
					return true;
			} else {
				if (backEdge[next] == true)
					return true;
			}
		}
		backEdge[s] = false;
	...
	static int[] topoSort(ArrayList<ArrayList<Integer>> g,int v,int indegree[]) {
		int res[]=new int[v];
