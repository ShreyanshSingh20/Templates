...
public class BridgesAndArticulationPoints {
	...
	public static void main(String[] args) {
		...
		int v = f.nextInt();
		ArrayList<ArrayList<Integer>> gArrayList = new ArrayList<>();
		for (int i = 0; i < v; i++) {
			gArrayList.add(new ArrayList<Integer>());
		}
		int e = f.nextInt();
		for (int i = 0; i < e; i++) {
			addEdge(gArrayList, f.nextInt(), f.nextInt());
		}
		int insertionTime[] = new int[v];
		int lowTime[] = new int[v];
		boolean vis[] = new boolean[v];
		boolean isArticulation[]=new boolean[v];
//		printBridges(gArrayList, 0, -1, insertionTime, lowTime, vis);
		printArticulation(gArrayList, 0, -1, insertionTime, lowTime, vis, isArticulation);
		for(int i=0;i<v;i++) {
			if(isArticulation[i]) p.print(i+" ");
		}
		p.close();
	...
	static void addEdge(ArrayList<ArrayList<Integer>> gArrayList, int u, int v) {
		gArrayList.get(u).add(v);
		gArrayList.get(v).add(u);
	}

	static void printBridges(ArrayList<ArrayList<Integer>> g, int node, int p, int insertionTime[], int lowTime[],
			boolean vis[]) {
		timer++;
		...
		for (int adjnode : g.get(node)) {
			...
			if (vis[adjnode] == false) {
				printBridges(g, adjnode, node, insertionTime, lowTime, vis);
				lowTime[node] = Math.min(lowTime[node], lowTime[adjnode]);
				if (insertionTime[node] < lowTime[adjnode]) {
	...
	static void printArticulation(ArrayList<ArrayList<Integer>> g, int node, int p, 
			int insertionTime[], int lowTime[],
			boolean vis[],boolean isArticulation[]) {
		timer++;
		insertionTime[node] = timer;
		lowTime[node] = timer;
		vis[node] = true;

		int child=0;//calculation of number of individual childs
		for (int adjnode : g.get(node)) {
			if (adjnode == p)
				continue;
			if (vis[adjnode] == false) {
				printArticulation(g, adjnode, node, insertionTime, lowTime, vis,isArticulation);
				lowTime[node] = Math.min(lowTime[node], lowTime[adjnode]);
				if (insertionTime[node] <= lowTime[adjnode] && p!=-1) {
		...
		if(p==-1 && child>1) isArticulation[node]=true;
	}
