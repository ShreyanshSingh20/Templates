...
import graph.RottingOranges.Pair;
...
public class CycleDetectionUndirected {
	...
	public static void main(String[] args) {
		...
		p.println(dfsCheck(g, n));
		p.println(bfsChecker(g, n));
	...
	static boolean dfsCheck(ArrayList<ArrayList<Integer>> g, int v) {
		boolean visited[] = new boolean[v];
		for (int i = 0; i < v; i++) {
			if (visited[i] == false) {
				if (dfsSolver(g, visited, i, -1))
					return true;
			}
		}
	...
	static boolean dfsSolver(ArrayList<ArrayList<Integer>> g, boolean visited[], int s, int parent) {
		visited[s] = true;
		for (int next : g.get(s)) {
			if (visited[next] == false) {
				if (dfsSolver(g, visited, next, s))
					return true;
			} else {
				if (next != parent)
					return true;
	...
	static boolean bfsChecker(ArrayList<ArrayList<Integer>> g, int v) {
		boolean visited[] = new boolean[v];
		for (int i = 0; i < v; i++) {
			if (visited[i] == false) {
				if (bfsSolver(g, i,visited))
					return true;
			}
		}
	...
	static boolean bfsSolver(ArrayList<ArrayList<Integer>> g,int s,boolean visited[]) {
