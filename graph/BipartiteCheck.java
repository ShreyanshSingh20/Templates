...
public class BipartiteCheck {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < e; i++) {
			addEdge(g, f.nextInt()-1, f.nextInt()-1);
		}
		isBipartite(g);
	...
	static  void isBipartite(ArrayList<ArrayList<Integer>> g) {
		PrintWriter p = new PrintWriter(System.out);
    ...
    static boolean validColor(ArrayList<ArrayList<Integer>> g, int[] colors, int color, int node) {
        if (colors[node] != 0)
            return colors[node] == color;
        colors[node] = color;
        for (int adjacent : g.get(node))
