...
public class RottingOranges {
	...
	static int bfs(int mat[][],Queue<Pair> queue,boolean visited[][]) {
		int ans=0;
		while(!queue.isEmpty()) {
			int l=queue.size();
			for(int i=0;i<l;i++) {
				...
				int currt=currPair.time;
				ans=Math.max(ans,currt);
				for(int j=0;j<4;j++) {
					int cx=currx+offx[j];
					int cy=curry+offy[j];
					if(cx<0||cy<0||cx>=mat.length||cy>=mat[0].length||mat[cx][cy]==0||visited[cx][cy]==true) continue;
					mat[cx][cy]=2;
					visited[cx][cy]=true;
					queue.add(new Pair(cx,cy,currt+1));
				}
			}
		}
		return ans;
