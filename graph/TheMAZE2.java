...
public class TheMAZE2 {
	...
	static int bfs(int mat[][],int sr,int sc,int dr,int dc) {
		...
		int sp[]=new int[mat.length*mat[0].length+1];
		Arrays.fill(sp,Integer.MAX_VALUE);
		sp[sr*mat[0].length+sc]=0;
		...
		while(!pq.isEmpty()) {
			...
			for(int j=0;j<4;j++) {
				...
				while(newx>=0&&newy>=0&&newx<mat.length&&newy<mat[0].length&&mat[newx][newy]==0) {
					newx+=offx[j];
					newy+=offy[j];
					cnt++;
				}
				...
				if(dist+cnt<sp[newx*mat[0].length+newy]) {
					sp[newx*mat[0].length+newy]=dist+cnt;
					pq.add(new Pair(newx, newy, sp[newx*mat[0].length+newy]));
				}
				
			}
		}
		
		if(sp[dr*mat[0].length+dc]==Integer.MAX_VALUE) return -1;
		else return sp[dr*mat[0].length+dc];
	}
