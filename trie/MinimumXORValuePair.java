...
public class MinimumXORValuePair {
	...
	static class Trie{
		...
		public int getMinXor(int num) {
			Node tempNode=rootNode;
			for(int bit=31;bit>=0;bit--) {
				int bitVal=getBitVal(num, bit);
				if(tempNode.getChild(bitVal)==null) {
					int rev=getReverse(bitVal);
					if(tempNode.getChild(rev)==null) return Integer.MAX_VALUE;
					else tempNode=tempNode.getChild(rev);
				}else {
					tempNode=tempNode.getChild(bitVal);
				}
			}
		...
		public int findMinXor(int arr[],int n) {
			int minXor=arr[0]^arr[1];
			insert(arr[0]);
			for(int i=1;i<n;i++) {
				minXor=Math.min(minXor,getMinXor(arr[i]));
				insert(arr[i]);
			}
			return minXor;
		}
