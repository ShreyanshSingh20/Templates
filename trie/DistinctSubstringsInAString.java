...
public class DistinctSubstringsInAString {
	...
	public static void main(String[] args) {
		...
		String string=f.nextLine();
		Node rootNode=new Node();
		ArrayList<String> list=new ArrayList<>();
		int res=count(string, rootNode, list);
	...
	static int count(String string,Node rootNode,ArrayList<String> res) {
		int cnt=0;
		for(int i=0;i<string.length();i++) {
			Node temp=rootNode;
			for(int j=i;j<string.length();j++) {
				if(temp.getChild(string.charAt(j)-'a')==null) {
					Node newNode=new Node();
					temp.setChild(string.charAt(j)-'a', newNode);
					temp.setFlag();
					cnt++;
					res.add(string.substring(i, j+1));
				}
				temp=temp.getChild(string.charAt(j)-'a');
			}
		}
	...
	static class Trie {
		...
		public void insert(String word) {
			...
			for (char c : word.toCharArray()) {
				if (curr.getChild(c - 'a') == null)
					curr.setChild(c - 'a', new Node());
				curr.incrementPreCount();
		...
		// Returns if the word is in the trie and its cnt
		...
		// that starts with the given prefix and its cnt
