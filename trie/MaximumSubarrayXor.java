...
public class MaximumSubarrayXor {
	...
	public static void main(String[] args) {
		...
		Trie trie=new Trie();
		int res=trie.findMaxSubarrayXor(arr, n);
		p.println(res);
		p.close();
	...
	static class Trie{
		...
		public int findMaxSubarrayXor(int arr[],int n) {
			int prefXor=arr[0];
			int maxXor=arr[0];
			insert(arr[0]);
			for(int i=1;i<n;i++) {
				prefXor=prefXor^arr[i];
				maxXor=Math.max(Math.max(maxXor, prefXor),Math.max(arr[i], getMaxXor(prefXor)));
				insert(prefXor);
				insert(arr[i]);
			}
