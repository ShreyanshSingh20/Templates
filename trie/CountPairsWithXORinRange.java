...
public class CountPairsWithXORinRange {
	...
	public static void main(String[] args) {
		...
		int high=f.nextInt();
		Trie trie=new Trie();
		int ans=trie.cntPairs(arr, n, low, high);
		p.println(ans);
		p.close();
	...
	static class Trie{
		...
		public int getCnt(int num,int x) {
			int res=0;
			Node tempNode=root;
			for(int i=31;i>=0;i--) {
				int xs=getBitVal(x, i);
				int nums=getBitVal(num,i);
				if(xs==1&&nums==1) {
					if(tempNode.getChild(1)!=null) {
						res+=tempNode.getChild(1).cnt;
					}
		...
		public int cntPairs(int arr[],int n,int low,int high) {
			insert(arr[0]);
			int res=0;
			for(int i=1;i<n;i++) {
				int b=getCnt(arr[i],high+1);
				int a=getCnt(arr[i], low);
				res+=(b-a);
				insert(arr[i]);
			}
			return res;
