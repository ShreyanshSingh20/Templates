...
public class MaximumCompatibilityScoreSum {
	...
	static void helper(int s[][], int m[][], boolean visited[], int pos, int score) {
		...
		for (int i = 0; i < m.length; i++) {
			if (visited[i] == false) {
				visited[i] = true;
				helper(s, m, visited, pos + 1, score + count(s[pos], m[i]));
				visited[i] = false;
			}
		}
	}

	static int count(int s[], int m[]) {
		int ans = 0;
