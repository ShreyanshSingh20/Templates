...
public class GroupAnagrams {
	...
	public static void main(String[] args) {
		...
		int x = 0;
		ans.get(0).add(arr[a.get(0).ind]);
		for (int i = 1; i < n; i++) {
			if (a.get(i).str.equals(a.get(i - 1).str)) {
				ans.get(x).add(arr[a.get(i).ind]);
			} else {
				x++;
				ans.add(new ArrayList<String>());
				ans.get(x).add(arr[a.get(i).ind]);
