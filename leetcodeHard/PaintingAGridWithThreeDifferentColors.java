...
public class PaintingAGridWithThreeDifferentColors {
	...
	public static void main(String[] args) {
		...
		int r = f.nextInt();
		int c = f.nextInt();
		long dp[][] = new long[1005][1030];
		for (long[] row : dp)
			Arrays.fill(row, -1);

		long res = ways(0, 0, dp, c, r);
		p.println(res);
		p.close();

	}

	static void dfs(int cell, long currMask, long prevMask, int r, ArrayList<Long> val) {
		if (cell == r) {
			val.add(currMask);
			return;
		}
		...
		for (int i = 1; i <= 3; i++) {
			if (i != get(cell, prevMask) && (r == 0 || i != get(cell - 1, currMask))) {
				dfs(cell + 1, set(cell, currMask, i), prevMask, r, val);
			}
		}
	}

	static long get(int cell, long mask) {
		return (mask >> 2 * cell) & 3;
	}

	static long set(int cell, long mask, int colour) {
		return (mask | (1 << 2 * cell));
	}

	static long ways(int i, long mask, long dp[][], int c, int r) {
		if (i == c)
			return 1%m;
		if (dp[i][(int) mask] != -1)
			return dp[i][(int) mask]%m;
		ArrayList<Long> val = new ArrayList<>();
		dfs(0, 0, mask, r, val);
		long ans = 0;
		for (long x : val) {
			ans = (ans % m + ways(i + 1, x, dp, c, r) % m) % m;
		}
		dp[i][(int) mask] = ans;
		return ans;
	}
