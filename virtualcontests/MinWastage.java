...
public class MinWastage {
	...
	static Integer[][] dp = new Integer[200][200];
	
	
	static int res(int arr[],int i,int k) {
		int n=arr.length;
		if(i==n) return 0;
		...
		for(int j=i;j<n;j++) {
			...
			totalSum+=arr[j];
			int waste=max*(j-i+1)-totalSum;
			ans=Math.min(ans,res(arr,j+1,k-1)+waste);
		}
