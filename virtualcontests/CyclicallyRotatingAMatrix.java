...
public class CyclicallyRotatingAMatrix {
    ...
    public static void main(String[] args) {
        ...
        for(int l=0;l<layers;l++) {
        	int arr[]=new int[2*r+2*c-4];
        	int ans[]=new int[2*r+2*c-4];
        	int plc=k%(2*r+2*c-4);
        	//left col
        	int pt=0;
        	for(int i=la;i<lb;i++) {
        	...
        	for(int i=0;i<arr.length;i++) {
        		ans[(i+plc)%ans.length]=arr[i];
        	}
        	
        	//left col
        	int ptd=0;
        	for(int i=lad;i<lbd;i++) {
        		res[i][ptld]=ans[ptd];
        		ptd++;
        	}
        	lad++;
    ...
    static void replace(int res[][],int ans[]) {
