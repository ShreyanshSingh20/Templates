...
public class MinimumCosttoReachDestinationinTime {
	...
	static class MinPair {
		...
		public MinPair(int vertex, int time, int cost) {
			super();
			this.time = time;
			this.vertex = vertex;
			this.cost = cost;
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < e; i++) {
			addEdge(a, f.nextInt(), f.nextInt(), f.nextInt());
		}
		...
		time[0] = 0;
		res[0] = cost[0];
		PriorityQueue<MinPair> pq = new PriorityQueue<MinPair>(1, new Comparator<MinPair>() { // overriding the compare
																								// method
			public int compare(MinPair i, MinPair j) {
				if (i.cost == j.cost) {
					return i.time - j.time;
				}
				return i.cost - j.time;
			}
		});

		pq.offer(new MinPair(0, 0, cost[0]));
		while (pq.isEmpty() == false) {
			MinPair curr = pq.poll();
			for (Pair next : a.get(curr.vertex)) {
				if (curr.time + next.time <= maxTime) {
					// relax on cost
					if (res[next.dest] > curr.cost + cost[next.dest]) {
						res[next.dest] = curr.cost + cost[next.dest];
						time[next.dest] = curr.time + next.time;
						pq.offer(new MinPair(next.dest, time[next.dest], res[next.dest]));
					} else if (time[next.dest] > curr.time + next.time) {
						time[next.dest] = curr.time + next.time;
						pq.offer(new MinPair(next.dest, time[next.dest], curr.cost + cost[next.dest]));
					}
	...
	static void addEdge(ArrayList<ArrayList<Pair>> a, int src, int dest, int time) {
		// for undirected graphs //can be changed
		a.get(src).add(new Pair(dest, time));
		a.get(dest).add(new Pair(src, time));
	}
