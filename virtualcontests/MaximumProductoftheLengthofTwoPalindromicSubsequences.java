...
public class MaximumProductoftheLengthofTwoPalindromicSubsequences {
	...
	static int solve(String a,String b,char c[],int i) {
		...
		ans=Math.max(ans,solve(a+c[i],b,c,i+1));
		ans=Math.max(ans,solve(a,b+c[i],c,i+1));
		ans=Math.max(ans,solve(a,b,c,i+1));
