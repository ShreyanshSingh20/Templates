...
public class LongestRepeatingSubsequenceKtimes {
    ...
    static int solve(int dp[][],char c[],int i,int j) {
    	if(dp[i][j]!=-1) return dp[i][j];
    	if(i>=c.length||j>=c.length) return 0;
    	
    	if(c[i]==c[j]&&i!=j) {
    		return dp[i][j]=1+solve(dp,c, i+1, j+1);
    	}else {
    		return dp[i][j]=Math.max(solve(dp,c, i+1, j), solve(dp,c, i, j+1));
    	}
    }
