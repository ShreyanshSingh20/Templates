...
public class RatInMaze {
	...
	static boolean path(int mat[][],int sol[][],int i,int j,int r,int c) {
		...
		if(isSafe(mat,sol,i,j,r,c)==true) {
			for(int off=0;off<8;off++) {
				sol[i][j]=1;
				if(path(mat, sol, i+offx[off], j+offy[off], r, c)) return true;
				sol[i][j]=0;
			}
		}
	...
	//1--traps
	//0--floor
	static boolean isSafe(int mat[][],int sol[][],int i,int j,int r,int c) {
		
		 if(i>r-1||j>c-1||i<0||j<0||sol[i][j]==1||mat[i][j]==1) return false;
		 else return true;
	}
