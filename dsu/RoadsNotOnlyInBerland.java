...
public class RoadsNotOnlyInBerland {
	...
	public static void main(String[] args) {
		...
		DSU dsu=new DSU(n);
		ArrayList<Pair> avaiList=new ArrayList<>();
		for(int i=0;i<n-1;i++) {
			int u=f.nextInt()-1;
			int v=f.nextInt()-1;
			addEdge(g, u, v);
			if(dsu.find(u)==dsu.find(v)) {
				avaiList.add(new Pair(u, v));
			}else dsu.union(u, v);
		...
		for(int i=0;i<n;i++) {
			parentSet.add(dsu.find(i));
		}
		
		p.println(parentSet.size()-1);
		ArrayList<Integer> toAddList=new ArrayList<>();
		Iterator<Integer> eIterator=parentSet.iterator();
		while(eIterator.hasNext()) {
			toAddList.add((int)eIterator.next());
		}
		
		int i=0;//pt on availlist
		for(int j=1;j<toAddList.size();j++) {
			int u=toAddList.get(j-1);
			int v=toAddList.get(j);
			int tu=avaiList.get(i).u;
			int tv=avaiList.get(i).v;
			p.print((tu+1)+" "+(tv+1)+" "+(u+1)+" "+(v+1));
			i++;
			p.println();
		}
