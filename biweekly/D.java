...
public class D {
	...
	public static void main(String[] args) {
		...
		int ones=0;
		for(int i=0;i<n;i++) {
			if(arr[i]%4==0||arr[i]%9==0||arr[i]==25) continue;
			int mask=0;
			for(int j=0;j<10;j++) {
				if(arr[i]%prime[j]==0) {
					mask=mask|(1<<j);
			...
			freq[mask]++;
			if(freq[mask]>1) continue;
			v.add(mask);
		}
		
		
		int dp[][]=new int[v.size()+1][1024];
		...
		int ans = solve(dp,v,freq,0,0);
        ans = (int) mul(binPower(2, ones,mod),ans);
		p.println(ans);
	...
	static int solve(int dp[][],ArrayList<Integer> v,int freq[],int pos,int mask) {
		
		if(pos==v.size()) return (mask>0?1:0);
		if(dp[pos][mask]!=-1) return (int) (dp[pos][mask]%mod);
		int ans=(int) (solve(dp,v,freq,pos+1,mask) % mod);
		if((v.get(pos)&mask)==0) ans = (int) add((int) mul(freq[v.get(pos)],solve(dp,v,freq,pos+1, mask|v.get(pos))),ans);
