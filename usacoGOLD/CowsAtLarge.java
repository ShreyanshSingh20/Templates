...
public class CowsAtLarge {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<n;i++) {
			if(deg[i]==1) exitPoints.add(i);
		}
		
		//find sp
		int sp[]=bfs(g, source, n);
		//finding shortest path from every exit point
		int spe[]=bfsexit(g, n, exitPoints);
		
		int res=ansBFS(g, source, n, sp, spe);
		p.println(res);
		p.close();
	...
	//bfs to find shortest dist from root to every node
	static int[] bfs(ArrayList<ArrayList<Integer>> g,int source,int n) {
	...
	static int ansBFS(ArrayList<ArrayList<Integer>> g,int source,int n,int sp[],int spe[]) {
		Queue<Integer> queue=new LinkedList<>();
		queue.add(source);
		boolean vis[]=new boolean[n];
		vis[source]=true;
		...
		while(!queue.isEmpty()) {
			...
			for(int child:g.get(parent)) {
				if(vis[child]==false) {
					if(sp[parent]<spe[parent]&&sp[child]>=spe[child]) res++;
					vis[child]=true;
					queue.add(child);
				}
			}
	...
	static int[] bfsexit(ArrayList<ArrayList<Integer>> g,int n,ArrayList<Integer> exitPoints) {
		int spe[]=new int[n];
		Arrays.fill(spe, Integer.MAX_VALUE);
		for(int i=0;i<exitPoints.size();i++) {
			bfsExitSolver(g, exitPoints.get(i), n, spe);
		}
	...
	static void bfsExitSolver(ArrayList<ArrayList<Integer>> g,int source,int n,int spe[]) {
		Queue<Integer> queue=new LinkedList<>();
		queue.add(source);
		boolean vis[]=new boolean[n];
		vis[source]=true;
